...
export default function TranscriptModal({ isOpen, onClose, certificate }) {
    ...
    useEffect(() => {
        if (isOpen && certificate) {
            setFetching(true);
            // Always fetch fresh from API to get transcript_data (list endpoint may not include it)
            api.get(`/certificates/${certificate.id}`)
                .then(res => {
                    const freshData = res.data?.transcript_data;
                    if (freshData) {
                        setData({
                            module_info: { ...defaultData.module_info, ...(freshData.module_info || {}) },
                            course_modules: freshData.course_modules || [],
                            competency_modules: freshData.competency_modules || []
                        });
                    } else {
                        setData(defaultData);
                    }
    ...
    return (
        <Dialog open={isOpen} onOpenChange={(val) => !val && onClose(false)}>
            <DialogContent className="max-w-5xl h-[85vh] flex flex-col">
                ...
                <div className="flex-1 overflow-y-auto pr-2 mt-4 custom-scrollbar relative">
                    {fetching ? (
                        <div className="flex items-center justify-center py-20 text-slate-400">
                            <div className="h-6 w-6 animate-spin rounded-full border-2 border-slate-300 border-t-blue-500 mr-3"></div>
                            Mevcut transkript yükleniyor...
                        </div>
                    ) : (
                ...
                <DialogFooter className="mt-6 shrink-0 pt-4 border-t gap-2">
